...
unsafe extern "C" fn x264_10_cavlc_init(mut h: *mut x264_t) {
    let mut i_suffix: c_int = 0;
    while i_suffix < 7 {
        let mut level: int16_t = (-LEVEL_TABLE_SIZE / 2) as int16_t;
        while (level as c_int) < LEVEL_TABLE_SIZE / 2 {
            let mut mask: c_int = level as c_int >> 15;
            ...
            let mut vlc: *mut vlc_large_t =
                &mut *(*x264_10_level_token.as_mut_ptr().offset(i_suffix as isize))
                    .as_mut_ptr()
                    .offset((level as c_int + LEVEL_TABLE_SIZE / 2) as isize)
                    as *mut vlc_large_t;
            if i_level_code >> i_suffix < 14 {
                (*vlc).i_size = ((i_level_code >> i_suffix) + 1 + i_suffix) as uint8_t;
                (*vlc).i_bits =
                    (((1) << i_suffix) + (i_level_code & ((1) << i_suffix) - 1)) as uint16_t;
            } else if i_suffix == 0 && i_level_code < 30 {
                (*vlc).i_size = 19;
                (*vlc).i_bits = (((1) << 4) + (i_level_code - 14)) as uint16_t;
            } else if i_suffix > 0 && i_level_code >> i_suffix == 14 {
                (*vlc).i_size = (15 + i_suffix) as uint8_t;
                (*vlc).i_bits =
                    (((1) << i_suffix) + (i_level_code & ((1) << i_suffix) - 1)) as uint16_t;
            } else {
                ...
                (*vlc).i_size = 28;
                (*vlc).i_bits = (((1) << 12) + i_level_code) as uint16_t;
            }
            if i_next == 0 {
                i_next += 1;
            }
    ...
    while i < ((1) << 16) as uint32_t {
        ...
        let mut j: c_int = 0;
        while j < 16 {
            dct[j as usize] = (i & ((1) << j) as uint32_t) as dctcoef;
            j += 1;
        }
        let mut total: c_int =
            (*h).quantf.coeff_level_run[DCT_LUMA_4x4 as c_int as usize]
                .expect("non-null function pointer")(dct.as_mut_ptr(), &mut runlevel);
        let mut zeros: c_int = runlevel.last as c_int + 1 - total;
        let mut mask_0: uint32_t = i << i.leading_zeros() as i32 + 1;
        let mut j_0: c_int = 0;
        while j_0 < total - 1 && zeros > 0 {
            let mut idx: c_int = (if zeros < 7 { zeros } else { 7 }) - 1;
        ...
        x264_10_run_before[i as usize] = ((bits << 5) + size) as uint32_t;
        i = i.wrapping_add(1);
    }
}
