                    ...
                    loop {
                        ...
                        if (*h).param.b_sliced_threads != 0 {
                            if h == (*h).thread[0usize] && i_0 == 0 {
                                (*h).deblock_strength[0usize] =
                                    crate::src::common::base::x264_malloc(
                                        (::core::mem::size_of::<
                                            [[[crate::stdlib::uint8_t; 4]; 8]; 2],
                                        >())
...
unsafe extern "C" fn macroblock_cache_load(
    ...
    unsafe {
        ...
        if b_mbaff != 0 {
            (*h).mb.pic.i_fref[0usize] = (*h).i_ref[0usize] << (*h).mb.b_interlaced;
            (*h).mb.pic.i_fref[1usize] = (*h).i_ref[1usize] << (*h).mb.b_interlaced;
        }
        if b_mbaff == 0 {
            x264_8_copy_column8(
