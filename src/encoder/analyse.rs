...
static mut i_sub_mb_b_cost_table: [uint8_t; 13] = [7, 5, 5, 3, 7, 5, 7, 3, 7, 7, 7, 5, 1];
#[c2rust::src_loc = "136:22"]
static mut i_sub_mb_p_cost_table: [uint8_t; 4] = [5, 3, 3, 1];
...
unsafe extern "C" fn init_costs(
    ...
    (*h).cost_mv[qp as usize] = x264_malloc(
        ((4 * 4 * mv_range + 1) as usize).wrapping_mul(size_of::<uint16_t>() as usize) as int64_t,
    ) as *mut uint16_t;
    if !(*h).cost_mv[qp as usize].is_null() {
        (*h).cost_mv[qp as usize] = (*h).cost_mv[qp as usize].offset((2 * 4 * mv_range) as isize);
        let mut i: c_int = 0;
        while i <= 2 * 4 * mv_range {
            let ref mut fresh2 = *(*h).cost_mv[qp as usize].offset(i as isize);
            *fresh2 = (if ((lambda as c_float * *logs.offset(i as isize) + 0.5f32) as c_int) < 65535
            {
        ...
        while i_0 < 3 {
            ...
            while j < 33 {
                (*(*h).cost_table).ref_0[qp as usize][i_0 as usize][j as usize] = (if i_0 != 0 {
                    if lambda * bs_size_te(i_0, j) < 65535 {
            ...
            && (*h).cost_mv_fpel[qp as usize][0].is_null()
        {
            ...
            loop {
                ...
                (*h).cost_mv_fpel[qp as usize][j_0 as usize] = x264_malloc(
                    ((4 * mv_range + 1) as usize).wrapping_mul(size_of::<uint16_t>() as usize)
                        as int64_t,
                ) as *mut uint16_t;
                ...
                (*h).cost_mv_fpel[qp as usize][j_0 as usize] =
                    (*h).cost_mv_fpel[qp as usize][j_0 as usize].offset((2 * mv_range) as isize);
                let mut i_1: c_int = -(2) * mv_range;
                while i_1 < 2 * mv_range {
                    *(*h).cost_mv_fpel[qp as usize][j_0 as usize].offset(i_1 as isize) =
                        *(*h).cost_mv[qp as usize].offset((i_1 * 4 + j_0) as isize);
                    i_1 += 1;
                }
                j_0 += 1;
            }
            ...
            _ => {
                ...
                while i_2 < 17 {
                    *cost_i4x4_mode.offset(i_2 as isize) =
                        (3 * lambda * (i_2 != 8) as c_int) as uint16_t;
                    i_2 += 1;
                }
                return 0;
            }
...
pub unsafe extern "C" fn x264_10_analyse_init_costs(mut h: *mut x264_t) -> c_int {
    ...
    *logs.offset(0) = 0.718f32;
    let mut i: c_int = 1;
    while i <= 2 * 4 * mv_range {
        *logs.offset(i as isize) = log2f((i + 1) as c_float) * 2.0f32 + 1.718f32;
        i += 1;
    }
    let mut qp: c_int = if (*h).param.rc.i_qp_min < 51 + 6 * (10 - 8) {
        (*h).param.rc.i_qp_min
    } else {
        51 + 6 * (10 - 8)
    };
...
pub unsafe extern "C" fn x264_10_analyse_free_costs(mut h: *mut x264_t) {
    ...
    while i < QP_MAX + 1 {
        if !(*h).cost_mv[i as usize].is_null() {
            x264_free(
                (*h).cost_mv[i as usize].offset(-((2 * 4 * mv_range) as isize)) as *mut c_void,
            );
        }
        let mut j: c_int = 0;
        while j < 4 {
            if !(*h).cost_mv_fpel[i as usize][j as usize].is_null() {
                x264_free(
                    (*h).cost_mv_fpel[i as usize][j as usize].offset(-((2 * mv_range) as isize))
                        as *mut c_void,
                );
            }
            j += 1;
...
pub unsafe extern "C" fn x264_10_analyse_weight_frame(mut h: *mut x264_t, mut end: c_int) {
    ...
    while j < (*h).i_ref[0] {
        if !(*h).sh.weight[j as usize][0].weightfn.is_null() {
            ...
            height = (if 16 + end + i_padv < (*(*h).fref[0][j as usize]).i_lines[0] + i_padv * 2 {
                16 + end + i_padv
            } else {
                (*(*h).fref[0][j as usize]).i_lines[0] + i_padv * 2
...
pub unsafe extern "C" fn mb_analyse_load_costs(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    .offset(
        (x264_clip3 as unsafe extern "C" fn(c_int, c_int, c_int) -> c_int)(
            (*h).sh.i_num_ref_idx_l0_active - 1,
            0,
            2,
        ) as isize,
    ))
    ...
    .offset(
        (x264_clip3 as unsafe extern "C" fn(c_int, c_int, c_int) -> c_int)(
            (*h).sh.i_num_ref_idx_l1_active - 1,
            0,
            2,
        ) as isize,
    ))
...
unsafe extern "C" fn mb_analyse_init_qp(
    ...
    (*h).mb.b_trellis = ((*h).param.analyse.i_trellis > 1 && (*a).i_mbrd != 0) as c_int;
    ...
    let mut chroma_offset_idx: c_int = if (qp - effective_chroma_qp + 12) < 36 {
        qp - effective_chroma_qp + 12
    } else {
        36
    };
    ...
    if qp > QP_MAX_SPEC {
        (*h).nr_offset = (*(*h)
            .nr_offset_emergency
            .offset((qp - QP_MAX_SPEC - 1) as isize))
...
unsafe extern "C" fn mb_analyse_init(
    ...
    (*a).i_mbrd = (subme >= 6) as c_int
        + (subme >= 8) as c_int
        + ((*h).param.analyse.i_subpel_refine >= 10) as c_int;
    (*h).mb.b_deblock_rdo = ((*h).param.analyse.i_subpel_refine >= 9
        && (*h).sh.i_disable_deblocking_filter_idc != 1) as c_int;
    (*a).b_early_terminate = ((*h).param.analyse.i_subpel_refine < 11) as c_int;
    ...
    let mut pcm_cost: uint64_t = ((256 * 10
        + 2 * (if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            256 * 10 >> (*h).mb.chroma_h_shift + (*h).mb.chroma_v_shift
        } else {
            0
        })
        + 16) as uint64_t)
    ...
    if (*h).sh.i_type != SLICE_TYPE_I as c_int {
        let mut i_fmv_range: c_int = 4 * (*h).param.analyse.i_mv_range;
        let mut i_fpel_border: c_int = 6;
        ...
        (*h).mb.mv_max_spel[0] = if (*h).mb.mv_max[0] < i_fmv_range - 1 {
            (*h).mb.mv_max[0]
        } else {
            i_fmv_range - 1
        };
        if (*h).param.intra_refresh && (*h).sh.i_type == SLICE_TYPE_P as c_int {
            let mut max_x: c_int = ((*(*h).fref[0][0]).i_pir_end_col * 16 - 3) * 4;
            let mut max_mv: c_int = max_x - 4 * 16 * (*h).mb.i_mb_x;
        ...
        (*h).mb.mv_limit_fpel[0][0] = (((*h).mb.mv_min_spel[0] >> 2) + i_fpel_border) as int16_t;
        (*h).mb.mv_limit_fpel[1][0] = (((*h).mb.mv_max_spel[0] >> 2) - i_fpel_border) as int16_t;
        if (*h).mb.i_mb_x == 0 && (*h).mb.i_mb_y & (*h).param.interlaced as i32 == 0 {
            ...
            if (*h).param.interlaced {
                let mut i_0: c_int = 0;
                while i_0 < 3 {
                    let mut j_0: c_int = (i_0 == 2) as c_int;
                    mb_y = ((*h).mb.i_mb_y >> j_0) + (i_0 == 1) as c_int;
                    (*h).mb.mv_miny_row[i_0 as usize] = 4 * (-(16) * mb_y - 24);
                    (*h).mb.mv_maxy_row[i_0 as usize] =
                        4 * (16 * (((*h).mb.i_mb_height >> j_0) - mb_y - 1) + 24);
                    ...
                    (*h).mb.mv_maxy_spel_row[i_0 as usize] = if (*h).mb.mv_maxy_row[i_0 as usize]
                        < (if (i_fmv_range - 1) < 4 * thread_mvy_range {
                            i_fmv_range - 1
                        } else {
                            4 * thread_mvy_range
                        }) {
                        (*h).mb.mv_maxy_row[i_0 as usize]
                    } else if (i_fmv_range - 1) < 4 * thread_mvy_range {
                        i_fmv_range - 1
                    } else {
                        4 * thread_mvy_range
                    };
                    (*h).mb.mv_miny_fpel_row[i_0 as usize] =
                        ((*h).mb.mv_miny_spel_row[i_0 as usize] >> 2) + i_fpel_border;
                    (*h).mb.mv_maxy_fpel_row[i_0 as usize] =
                        ((*h).mb.mv_maxy_spel_row[i_0 as usize] >> 2) - i_fpel_border;
                    i_0 += 1;
                }
            } else {
                ...
                (*h).mb.mv_limit_fpel[0][1] =
                    (((*h).mb.mv_min_spel[1] >> 2) + i_fpel_border) as int16_t;
                (*h).mb.mv_limit_fpel[1][1] =
                    (((*h).mb.mv_max_spel[1] >> 2) - i_fpel_border) as int16_t;
            }
        }
        if (*h).param.interlaced {
        ...
        if (*a).b_early_terminate != 0 && (*h).mb.i_mb_xy - (*h).sh.i_first_mb > 4 {
            if !((*h).mb.i_mb_type_left[0] == I_4x4 as c_int
                ...
                || (*h).mb.i_mb_xy - (*h).sh.i_first_mb
                    < 3 * ((*h).stat.frame.i_mb_count[I_4x4 as c_int as usize]
...
unsafe extern "C" fn predict_8x8_mode_available(
    ...
    let mut avoid_topright: c_int = (force_intra != 0 && i & 1 != 0) as c_int;
    let mut idx: c_int = i_neighbour & (MB_TOP as c_int | MB_LEFT as c_int | MB_TOPLEFT as c_int);
    idx = if idx == MB_TOP as c_int | MB_LEFT as c_int | MB_TOPLEFT as c_int {
        4
    } else {
...
unsafe extern "C" fn predict_4x4_mode_available(
    ...
    let mut avoid_topright: c_int = (force_intra != 0 && i & 5 == 5) as c_int;
    let mut idx: c_int = i_neighbour & (MB_TOP as c_int | MB_LEFT as c_int | MB_TOPLEFT as c_int);
    idx = if idx == MB_TOP as c_int | MB_LEFT as c_int | MB_TOPLEFT as c_int {
        4
    } else {
...
unsafe extern "C" fn mb_init_fenc_cache(mut h: *mut x264_t, mut b_satd: c_int) {
    if (*h).param.analyse.i_trellis == 2 && (*h).mb.i_psy_trellis != 0 {
...
unsafe extern "C" fn mb_analyse_intra_chroma(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
        ...
        if (*h).mb.lossless {
            x264_10_predict_lossless_16x16(h, 1, (*a).i_predict16x16);
            x264_10_predict_lossless_16x16(h, 2, (*a).i_predict16x16);
        } else {
...
unsafe extern "C" fn mb_analyse_intra(
    ...
    if (*h).param.i_avcintra_class == 0 {
        ...
        static mut i16x16_thresh_lut: [uint8_t; 11] = [2, 2, 2, 3, 3, 4, 4, 4, 4, 4, 4];
        let mut i16x16_thresh: c_int = if (*a).b_fast_intra != 0 {
            i16x16_thresh_lut[(*h).mb.i_subpel_refine as usize] as c_int * i_satd_inter >> 1
        } else {
            COST_MAX
        };
        if !(*h).mb.lossless && *predict_mode.offset(3) >= 0 {
            ...
            if (*a).i_satd_i16x16 <= i16x16_thresh {
                ...
                (*a).i_satd_i16x16_dir[I_PRED_16x16_P as c_int as usize] += lambda * bs_size_ue(3);
            ...
            while *predict_mode as c_int >= 0 {
                ...
                let mut i_mode: c_int = *predict_mode as c_int;
                if (*h).mb.lossless {
                    x264_10_predict_lossless_16x16(h, 0, i_mode);
                } else {
    ...
    if flags & X264_ANALYSE_I8x8 != 0 {
        ...
        loop {
            ...
            let mut p_src_by: *mut pixel = p_src
                .offset((8 * x) as isize)
                .offset((8 * y * FENC_STRIDE) as isize);
            let mut p_dst_by: *mut pixel = p_dst
                .offset((8 * x) as isize)
                .offset((8 * y * FDEC_STRIDE) as isize);
            let mut i_best: c_int = COST_MAX;
            let mut i_pred_mode: c_int = x264_mb_predict_intra4x4_mode(h, 4 * idx);
            ...
            if (*h).pixf.intra_mbcmp_x9_8x8.is_some() && *predict_mode_0.offset(8) as c_int >= 0 {
                i_best = (*h)
                    .pixf
                    .intra_mbcmp_x9_8x8
                    .expect("non-null function pointer")(
                ...
                if !(*h).mb.lossless && *predict_mode_0.offset(5) as c_int >= 0 {
                    ...
                    if i_pred_mode < 3 {
                        satd[i_pred_mode as usize] =
                            (satd[i_pred_mode as usize] as c_int - 3 * lambda) as int32_t;
                    }
                    let mut i: c_int = 2;
                    while i >= 0 {
                        let mut cost: c_int = satd[i as usize];
                        (*a).i_satd_i8x8_dir[idx as usize][i as usize] =
                            (cost + 4 * lambda) as uint16_t;
                        if cost < i_best {
                            i_best = cost;
                    ...
                    if (*a).i_mbrd < 1 + (*a).b_fast_intra {
                        ...
                        .as_ptr()
                        .offset((*predict_mode_0.offset(8) as c_int >= 0) as c_int as isize))
                        .as_ptr()
                ...
                while *predict_mode_0 as c_int >= 0 && (i_best >= 0 || (*a).i_mbrd >= 2) {
                    let mut i_satd_0: c_int = 0;
                    ...
                    if i_pred_mode == x264_mb_pred_mode4x4_fix[(i_mode_0 + 1) as usize] as c_int {
                        i_satd_0 -= 3 * lambda;
                    }
                    if i_satd_0 < i_best {
                        i_best = i_satd_0;
        ...
        if idx == 3 {
            ...
            if (*h).mb.i_skip_intra != 0 {
                ...
                (*h).mb.pic.i8x8_nnz_buf[3] = (*(&mut *(*h)
                    ...
                    .non_zero_count
                    .as_mut_ptr()
                    .offset(*x264_scan8.as_ptr().offset(10 as isize) as isize)
                    as *mut uint8_t
                    as *mut x264_union32_t))
                    .i;
            ...
            static mut cost_div_fix8: [uint16_t; 3] =
                [1024 as uint16_t, 512 as uint16_t, 341 as uint16_t];
            (*a).i_satd_i8x8 = COST_MAX;
            i_cost = i_cost * cost_div_fix8[idx as usize] as c_int >> 8;
        }
        static mut i8x8_thresh: [uint8_t; 11] = [4, 4, 4, 5, 5, 5, 6, 6, 6, 6, 6];
        if (*a).b_early_terminate != 0
            && (if i_cost < (*a).i_satd_i16x16 {
                i_cost
            } else {
                (*a).i_satd_i16x16
            }) > i_satd_inter * i8x8_thresh[(*h).mb.i_subpel_refine as usize] as c_int >> 2
        {
            return;
        }
    }
    if flags & X264_ANALYSE_I4x4 != 0 {
        let mut i_cost_0: c_int = lambda * (24 + 16);
        ...
        if (*a).b_early_terminate != 0 && (*a).i_mbrd != 0 {
            i_satd_thresh_0 = i_satd_thresh_0 * (10 - (*a).b_fast_intra) / 8;
        }
        if (*h).sh.i_type == SLICE_TYPE_B as c_int {
        ...
        loop {
            ...
            {
                (*(&mut *p_dst_by_0.offset((4 - 32) as isize) as *mut pixel
                    as *mut x264_union64_t))
                    .i = (*p_dst_by_0.offset((3 - 32) as isize) as c_ulonglong)
                    .wrapping_mul(0x1000100010001 as c_ulonglong)
                    as uint64_t;
            }
            if (*h).pixf.intra_mbcmp_x9_4x4.is_some() && *predict_mode_1.offset(8) as c_int >= 0 {
                i_best_0 = (*h)
                    .pixf
                    .intra_mbcmp_x9_4x4
                    .expect("non-null function pointer")(
                ...
                if i_cost_0 > i_satd_thresh_0 || idx == 15 {
                    break;
                }
                (*h).mb.cache.intra4x4_pred_mode[x264_scan8[idx as usize] as usize] =
                    i_best_0 as int8_t;
            } else {
                if !(*h).mb.lossless && *predict_mode_1.offset(5) as c_int >= 0 {
                    ...
                    if i_pred_mode_0 < 3 {
                        satd_0[i_pred_mode_0 as usize] =
                            (satd_0[i_pred_mode_0 as usize] as c_int - 3 * lambda) as int32_t;
                    }
                    ...
                    if (*a).i_mbrd < 1 + (*a).b_fast_intra {
                        predict_mode_1 = (*(*(*intra_analysis_shortcut
                            ...
                            .offset((*a).b_avoid_topright as isize))
                        .as_ptr()
                        .offset((*predict_mode_1.offset(8) as c_int >= 0) as c_int as isize))
                        .as_ptr()
                ...
                if i_best_0 > 0 {
                    while *predict_mode_1 as c_int >= 0 {
                        let mut i_satd_1: c_int = 0;
                        let mut i_mode_1: c_int = *predict_mode_1 as c_int;
                        if (*h).mb.lossless {
                            x264_10_predict_lossless_4x4(h, p_dst_by_0, 0, idx, i_mode_1);
                        } else {
                        ...
                        if i_pred_mode_0
                            == x264_mb_pred_mode4x4_fix[(i_mode_1 + 1) as usize] as c_int
                        {
                            i_satd_1 -= lambda * 3;
                            if i_satd_1 <= 0 {
                                i_best_0 = i_satd_1;
            ...
            x264_mb_encode_i4x4(h, 0, idx, (*a).i_qp, (*a).i_predict4x4[idx as usize], 0);
            idx += 1;
        }
        if idx == 15 {
...
unsafe extern "C" fn intra_rd_refine(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    if (*h).mb.i_type == I_16x16 as c_int {
        ...
        let mut i_thresh: c_int = if (*a).b_early_terminate != 0 {
            (*a).i_satd_i16x16_dir[old_pred_mode as usize] * 9 / 8
        } else {
            COST_MAX
        };
        ...
        || (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_422 as c_int
    {
        let mut predict_mode_0: *const int8_t =
            predict_chroma_mode_available((*h).mb.i_neighbour_intra as c_int);
        if *predict_mode_0.offset(1) as c_int >= 0 {
            ...
            i_max = 0;
            while *predict_mode_0 as c_int >= 0 {
                let mut i_mode_0: c_int = *predict_mode_0 as c_int;
    ...
    if (*h).mb.i_type == I_4x4 as c_int {
        let mut pels: [[pixel4; 4]; 3] = [[0 as pixel4, 0, 0, 0], [0; 4], [0; 4]];
        let mut nnz: [c_int; 3] = [0; 3];
        let mut idx: c_int = 0;
        while idx < 16 {
                ...
                == MB_TOP as c_int as c_uint
            {
                let mut p: c_int = 0;
                while p < plane_count {
                    (*(dst[p as usize].offset(4).offset(-(32 as isize)) as *mut x264_union64_t))
                        .i = (*dst[p as usize].offset((3 - 32) as isize) as c_ulonglong)
                        .wrapping_mul(0x1000100010001 as c_ulonglong)
                        as uint64_t;
                    p += 1;
                }
            ...
            while *predict_mode_1 as c_int >= 0 {
                ...
                if i_best > i_satd {
                    ...
                    while p_0 < plane_count {
                        pels[p_0 as usize][0] = (*(dst[p_0 as usize].offset((0 * 32) as isize)
                            as *mut x264_union64_t))
                            .i as pixel4;
                        pels[p_0 as usize][1] = (*(dst[p_0 as usize].offset((1 * 32) as isize)
                            as *mut x264_union64_t))
                            .i as pixel4;
                        pels[p_0 as usize][2] = (*(dst[p_0 as usize].offset((2 * 32) as isize)
                            as *mut x264_union64_t))
                            .i as pixel4;
                        pels[p_0 as usize][3] = (*(dst[p_0 as usize].offset((3 * 32) as isize)
                            as *mut x264_union64_t))
                            .i as pixel4;
                        nnz[p_0 as usize] = (*h).mb.cache.non_zero_count
                            [x264_scan8[(idx + p_0 * 16) as usize] as usize]
                            as c_int;
                        p_0 += 1;
                    }
                }
            ...
            while p_1 < plane_count {
                (*(dst[p_1 as usize].offset((0 * 32) as isize) as *mut x264_union64_t)).i =
                    pels[p_1 as usize][0] as uint64_t;
                (*(dst[p_1 as usize].offset((1 * 32) as isize) as *mut x264_union64_t)).i =
                    pels[p_1 as usize][1] as uint64_t;
                (*(dst[p_1 as usize].offset((2 * 32) as isize) as *mut x264_union64_t)).i =
                    pels[p_1 as usize][2] as uint64_t;
                (*(dst[p_1 as usize].offset((3 * 32) as isize) as *mut x264_union64_t)).i =
                    pels[p_1 as usize][3] as uint64_t;
                (*h).mb.cache.non_zero_count[x264_scan8[(idx + p_1 * 16) as usize] as usize] =
        ...
        let mut pels_h: [[pixel4; 2]; 3] = [[0 as pixel4, 0], [0; 2], [0; 2]];
        let mut pels_v: [[pixel; 7]; 3] = [[0 as pixel, 0, 0, 0, 0, 0, 0], [0; 7], [0; 7]];
        let mut nnz_0: [[uint16_t; 2]; 3] = [[0 as uint16_t, 0], [0; 2], [0; 2]];
        let mut idx_0: c_int = 0;
        while idx_0 < 4 {
            let mut x: c_int = idx_0 & 1;
            let mut y: c_int = idx_0 >> 1;
            ...
            while *predict_mode_2 as c_int >= 0 {
                ...
                if !((*a).i_satd_i8x8_dir[idx_0 as usize][i_mode_3 as usize] as c_int > i_thresh_1)
                {
                    ...
                    if i_best > i_satd {
                        ...
                        while p_3 < plane_count {
                            pels_h[p_3 as usize][0] =
                                (*(dst_0[p_3 as usize].offset((7 * 32) as isize).offset(0)
                                    as *mut x264_union64_t))
                                    .i as pixel4;
                            pels_h[p_3 as usize][1] =
                                (*(dst_0[p_3 as usize].offset((7 * 32) as isize).offset(4)
                                    as *mut x264_union64_t))
                                    .i as pixel4;
                            if idx_0 & 1 == 0 {
                                let mut j: c_int = 0;
                                while j < 7 {
                                    pels_v[p_3 as usize][j as usize] =
                                        *dst_0[p_3 as usize].offset((7 + j * FDEC_STRIDE) as isize);
                                    j += 1;
                                }
                            }
                            nnz_0[p_3 as usize][0] = (*(&mut *(*h)
                                .mb
                                .cache
                                .non_zero_count
                                .as_mut_ptr()
                                .offset((s8 + 0 * 8 + p_3 * 16) as isize)
                                as *mut uint8_t
                                as *mut x264_union16_t))
                                .i;
                            nnz_0[p_3 as usize][1] = (*(&mut *(*h)
                                .mb
                                .cache
                                .non_zero_count
                                .as_mut_ptr()
                                .offset((s8 + 1 * 8 + p_3 * 16) as isize)
                                as *mut uint8_t
                                as *mut x264_union16_t))
                                .i;
                            p_3 += 1;
            ...
            while p_4 < plane_count {
                (*(dst_0[p_4 as usize].offset((7 * 32) as isize).offset(0)
                    as *mut x264_union64_t))
                    .i = pels_h[p_4 as usize][0] as uint64_t;
                (*(dst_0[p_4 as usize].offset((7 * 32) as isize).offset(4)
                    as *mut x264_union64_t))
                    .i = pels_h[p_4 as usize][1] as uint64_t;
                if idx_0 & 1 == 0 {
                    let mut j_0: c_int = 0;
                    while j_0 < 7 {
                        *dst_0[p_4 as usize].offset((7 + j_0 * FDEC_STRIDE) as isize) =
                ...
                (*(&mut *(*h)
                    ...
                    .non_zero_count
                    .as_mut_ptr()
                    .offset((s8 + 0 * 8 + p_4 * 16) as isize) as *mut uint8_t
                    as *mut x264_union16_t))
                ...
                (*(&mut *(*h)
                    ...
                    .non_zero_count
                    .as_mut_ptr()
                    .offset((s8 + 1 * 8 + p_4 * 16) as isize) as *mut uint8_t
                    as *mut x264_union16_t))
                    .i = nnz_0[p_4 as usize][1];
                p_4 += 1;
            }
            x264_macroblock_cache_intra8x8_pred(h, 2 * x, 2 * y, (*a).i_predict8x8[idx_0 as usize]);
            idx_0 += 1;
        }
    }
}
...
unsafe extern "C" fn mb_analyse_inter_p16x16(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    let mut p_halfpel_thresh: *mut c_int =
        if (*a).b_early_terminate != 0 && (*h).mb.pic.i_fref[0] > 1 {
    ...
    m.p_fenc[0] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(0))
        .offset((0 + 0 * FENC_STRIDE) as isize) as *mut pixel;
    if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
        m.p_fenc[1] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(1)).offset(
            ((0 >> (*h).mb.chroma_h_shift) + (0 >> (*h).mb.chroma_v_shift) * FENC_STRIDE) as isize,
        ) as *mut pixel;
    ...
    while i_ref < (*h).mb.pic.i_fref[0] {
        ...
        .as_mut_ptr()
        .offset(0))
        .offset((0 + 0 * *m.i_stride.as_mut_ptr().offset(0)) as isize)
            as *mut pixel;
        ...
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
            ...
            .as_mut_ptr()
            .offset(4))
            .offset((0 + 0 * *m.i_stride.as_mut_ptr().offset(1)) as isize)
                as *mut pixel;
            ...
            .as_mut_ptr()
            .offset(8))
            .offset((0 + 0 * *m.i_stride.as_mut_ptr().offset(2)) as isize)
                as *mut pixel;
            if (*h).param.analyse.i_subpel_refine != 0 {
            ...
            .offset(4))
            .offset(
                (0 + (0 >> (*h).mb.chroma_v_shift) * *m.i_stride.as_mut_ptr().offset(1)) as isize,
            ) as *mut pixel;
        }
        if (*h).param.analyse.me_method.exhaustive_search() {
        ...
        x264_10_mb_predict_mv_16x16(h, 0, i_ref, m.mvp.as_mut_ptr());
        ...
        if i_ref == 0
            ...
            && abs(m.mv[0] as c_int - (*h).mb.cache.pskip_mv[0] as c_int)
                ...
                <= 1
            && x264_10_macroblock_probe_skip(h, 0) != 0
        {
    ...
    x264_macroblock_cache_ref(h, 0, 0, 4, 4, 0, (*a).l0.me16x16.i_ref as int8_t);
    if (*a).l0.me16x16.mv[1] as c_int <= (*h).mb.mv_max_spel[1] || (*h).i_thread_frames == 1 {
    } else {
        __assert_fail(
    ...
    if (*a).i_mbrd != 0 {
        mb_init_fenc_cache(
            h,
            ((*a).i_mbrd >= 2 || (*h).param.analyse.inter & X264_ANALYSE_PSUB8x8 != 0) as c_int,
        );
...
unsafe extern "C" fn mb_analyse_inter_p8x8_mixed_ref(
    ...
    let mut i_maxref: c_int = (*h).mb.pic.i_fref[0] - 1;
    (*h).mb.i_partition = D_8x8 as c_int;
    if (*a).b_early_terminate != 0
        && (i_maxref > 0
            && ((*a).l0.me16x16.i_ref == 0 || (*a).l0.me16x16.i_ref == (*h).mb.ref_blind_dupe)
            && (*h).mb.i_mb_type_top > 0
            && (*h).mb.i_mb_type_left[0] > 0)
    {
        i_maxref = 0;
        let mut ref_0: c_int = (*h).mb.cache.ref_0[0][(X264_SCAN8_0 + -(8) - 1) as usize] as c_int;
        if ref_0 > i_maxref && ref_0 != (*h).mb.ref_blind_dupe {
            i_maxref = ref_0;
        }
        let mut ref_1: c_int = (*h).mb.cache.ref_0[0][(X264_SCAN8_0 + -(8) + 0) as usize] as c_int;
        if ref_1 > i_maxref && ref_1 != (*h).mb.ref_blind_dupe {
            i_maxref = ref_1;
        }
        let mut ref_2: c_int = (*h).mb.cache.ref_0[0][(X264_SCAN8_0 + -(8) + 2) as usize] as c_int;
        if ref_2 > i_maxref && ref_2 != (*h).mb.ref_blind_dupe {
            i_maxref = ref_2;
        }
        let mut ref_3: c_int = (*h).mb.cache.ref_0[0][(X264_SCAN8_0 + -(8) + 4) as usize] as c_int;
        if ref_3 > i_maxref && ref_3 != (*h).mb.ref_blind_dupe {
            i_maxref = ref_3;
        }
        let mut ref_4: c_int = (*h).mb.cache.ref_0[0][(X264_SCAN8_0 + 0 - 1) as usize] as c_int;
        if ref_4 > i_maxref && ref_4 != (*h).mb.ref_blind_dupe {
            i_maxref = ref_4;
        }
        let mut ref_5: c_int = (*h).mb.cache.ref_0[0][(X264_SCAN8_0 + 2 * 8 - 1) as usize] as c_int;
    ...
    while i < 4 {
        ...
        let mut x8: c_int = i & 1;
        let mut y8: c_int = i >> 1;
        m.i_pixel = PIXEL_8x8 as c_int;
        m.p_cost_mv = (*a).p_cost_mv;
        m.i_stride[0] = (*h).mb.pic.i_stride[0];
        m.i_stride[1] = (*h).mb.pic.i_stride[1];
        m.i_stride[2] = (*h).mb.pic.i_stride[2];
        m.p_fenc[0] = &mut *(*p_fenc.offset(0)).offset((8 * x8 + 8 * y8 * FENC_STRIDE) as isize)
            as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            m.p_fenc[1] = &mut *(*p_fenc.offset(1)).offset(
                ((8 * x8 >> (*h).mb.chroma_h_shift)
                    + (8 * y8 >> (*h).mb.chroma_v_shift) * FENC_STRIDE) as isize,
            ) as *mut pixel;
        ...
        while i_ref_0 <= i_maxref || i_ref_0 == (*h).mb.ref_blind_dupe {
            ...
            .as_mut_ptr()
            .offset(0))
            .offset((8 * x8 + 8 * y8 * *m.i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            m.p_fref_w = m.p_fref[0];
            if (*h).param.analyse.i_subpel_refine != 0 {
                m.p_fref[1] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
                ...
                .as_mut_ptr()
                .offset(4))
                .offset((8 * x8 + 8 * y8 * *m.i_stride.as_mut_ptr().offset(1)) as isize)
                    as *mut pixel;
                m.p_fref[8] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref_0 as isize))
                .as_mut_ptr()
                .offset(8))
                .offset((8 * x8 + 8 * y8 * *m.i_stride.as_mut_ptr().offset(2)) as isize)
                    as *mut pixel;
                if (*h).param.analyse.i_subpel_refine != 0 {
                    m.p_fref[5] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                        .as_mut_ptr()
                ...
                .offset(
                    (8 * x8
                        + (8 * y8 >> (*h).mb.chroma_v_shift) * *m.i_stride.as_mut_ptr().offset(1))
                        as isize,
                ) as *mut pixel;
            }
            if (*h).param.analyse.me_method.exhaustive_search() {
            ...
            m.weight = x264_zero.as_mut_ptr() as *const x264_weight_t;
            m.i_ref = i_ref_0;
            m.p_fref_w = &mut *(*(*h).mb.pic.p_fref_w.as_mut_ptr().offset(i_ref_0 as isize))
                .offset((8 * x8 + 8 * y8 * *m.i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            m.weight = (*(*h).sh.weight.as_mut_ptr().offset(i_ref_0 as isize)).as_mut_ptr();
            x264_macroblock_cache_ref(h, 2 * x8, 2 * y8, 2, 2, 0, i_ref_0 as int8_t);
            x264_10_mb_predict_mv(h, 0, 4 * i, 2, m.mvp.as_mut_ptr());
        ...
        x264_macroblock_cache_ref(h, 2 * x8, 2 * y8, 2, 2, 0, (*l0m).i_ref as int8_t);
...
unsafe extern "C" fn mb_analyse_inter_p16x8(
    ...
    while i < 2 {
        ...
        let minref: c_int =
            if (*a).l0.me8x8[(2 * i) as usize].i_ref < (*a).l0.me8x8[(2 * i + 1) as usize].i_ref {
                (*a).l0.me8x8[(2 * i) as usize].i_ref
            } else {
                (*a).l0.me8x8[(2 * i + 1) as usize].i_ref
            };
        let maxref: c_int =
            if (*a).l0.me8x8[(2 * i) as usize].i_ref > (*a).l0.me8x8[(2 * i + 1) as usize].i_ref {
                (*a).l0.me8x8[(2 * i) as usize].i_ref
        ...
        let i_ref8s: c_int = if ref8[0] == ref8[1] { 1 } else { 2 };
        ...
        m.i_stride[2] = (*h).mb.pic.i_stride[2];
        m.p_fenc[0] =
            &mut *(*p_fenc.offset(0)).offset((0 + 8 * i * FENC_STRIDE) as isize) as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            m.p_fenc[1] = &mut *(*p_fenc.offset(1)).offset(
                ((0 >> (*h).mb.chroma_h_shift) + (8 * i >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
                    as isize,
            ) as *mut pixel;
            m.p_fenc[2] = &mut *(*p_fenc.offset(2)).offset(
                ((0 >> (*h).mb.chroma_h_shift) + (8 * i >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
        ...
        while j < i_ref8s {
            ...
            .as_mut_ptr()
            .offset(0))
            .offset((0 + 8 * i * *m.i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            m.p_fref_w = m.p_fref[0];
            if (*h).param.analyse.i_subpel_refine != 0 {
                m.p_fref[1] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
                ...
                .as_mut_ptr()
                .offset(4))
                .offset((0 + 8 * i * *m.i_stride.as_mut_ptr().offset(1)) as isize)
                    as *mut pixel;
                m.p_fref[8] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
                .as_mut_ptr()
                .offset(8))
                .offset((0 + 8 * i * *m.i_stride.as_mut_ptr().offset(2)) as isize)
                    as *mut pixel;
                if (*h).param.analyse.i_subpel_refine != 0 {
                    m.p_fref[5] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                        .as_mut_ptr()
                ...
                .offset(4))
                .offset(
                    (0 + (8 * i >> (*h).mb.chroma_v_shift) * *m.i_stride.as_mut_ptr().offset(1))
                        as isize,
                ) as *mut pixel;
            }
            if (*h).param.analyse.me_method.exhaustive_search() {
            ...
            m.p_fref_w = &mut *(*(*h).mb.pic.p_fref_w.as_mut_ptr().offset(i_ref as isize))
                ...
                as *mut pixel;
            m.weight = (*(*h).sh.weight.as_mut_ptr().offset(i_ref as isize)).as_mut_ptr();
            x264_macroblock_cache_ref(h, 0, 2 * i, 4, 2, 0, i_ref as int8_t);
            x264_10_mb_predict_mv(h, 0, 8 * i, 4, m.mvp.as_mut_ptr());
        ...
        if (*a).b_early_terminate != 0
            && (i == 0
                && (*l0m).cost + (*a).i_cost_est16x8[1]
                    > i_best_satd * (4 + ((*a).i_mbrd != 0) as c_int) / 4)
        {
        ...
        x264_macroblock_cache_ref(h, 0, 2 * i, 4, 2, 0, (*l0m).i_ref as int8_t);
        i += 1;
    }
...
unsafe extern "C" fn mb_analyse_inter_p8x16(
    ...
    while i < 2 {
        ...
        let minref: c_int =
            if (*a).l0.me8x8[i as usize].i_ref < (*a).l0.me8x8[(i + 2) as usize].i_ref {
                (*a).l0.me8x8[i as usize].i_ref
            } else {
                (*a).l0.me8x8[(i + 2) as usize].i_ref
            };
        let maxref: c_int =
            if (*a).l0.me8x8[i as usize].i_ref > (*a).l0.me8x8[(i + 2) as usize].i_ref {
                (*a).l0.me8x8[i as usize].i_ref
        ...
        m.i_stride[2] = (*h).mb.pic.i_stride[2];
        m.p_fenc[0] =
            &mut *(*p_fenc.offset(0)).offset((8 * i + 0 * FENC_STRIDE) as isize) as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            m.p_fenc[1] = &mut *(*p_fenc.offset(1)).offset(
                ((8 * i >> (*h).mb.chroma_h_shift) + (0 >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
                    as isize,
            ) as *mut pixel;
            m.p_fenc[2] = &mut *(*p_fenc.offset(2)).offset(
                ((8 * i >> (*h).mb.chroma_h_shift) + (0 >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
        ...
        while j < i_ref8s {
            ...
            .as_mut_ptr()
            .offset(0))
            .offset((8 * i + 0 * *m.i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            m.p_fref_w = m.p_fref[0];
            if (*h).param.analyse.i_subpel_refine != 0 {
                m.p_fref[1] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
                ...
                .as_mut_ptr()
                .offset(4))
                .offset((8 * i + 0 * *m.i_stride.as_mut_ptr().offset(1)) as isize)
                    as *mut pixel;
                m.p_fref[8] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
                .as_mut_ptr()
                .offset(8))
                .offset((8 * i + 0 * *m.i_stride.as_mut_ptr().offset(2)) as isize)
                    as *mut pixel;
                if (*h).param.analyse.i_subpel_refine != 0 {
                    m.p_fref[5] = &mut *(*(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                        .as_mut_ptr()
                ...
                .offset(4))
                .offset(
                    (8 * i + (0 >> (*h).mb.chroma_v_shift) * *m.i_stride.as_mut_ptr().offset(1))
                        as isize,
                ) as *mut pixel;
            }
            if (*h).param.analyse.me_method.exhaustive_search() {
            ...
            m.p_fref_w = &mut *(*(*h).mb.pic.p_fref_w.as_mut_ptr().offset(i_ref as isize))
                ...
                as *mut pixel;
            m.weight = (*(*h).sh.weight.as_mut_ptr().offset(i_ref as isize)).as_mut_ptr();
            x264_macroblock_cache_ref(h, 2 * i, 0, 2, 4, 0, i_ref as int8_t);
            x264_10_mb_predict_mv(h, 0, 4 * i, 2, m.mvp.as_mut_ptr());
            if (*h).mb.ref_blind_dupe == i_ref && ref8[0] == 0 {
                x264_10_me_refine_qpel_refdupe(h, &mut m, 0 as *mut c_int);
            } else {
        ...
        x264_macroblock_cache_ref(h, 2 * i, 0, 2, 4, 0, (*l0m).i_ref as int8_t);
        i += 1;
    }
...
unsafe extern "C" fn mb_analyse_inter_p4x4_chroma_internal(
    ...
    let mut or: c_int = 8 * (i8x8 & 1) + (4 >> chroma_v_shift) * (i8x8 & 2) * i_stride;
    ...
    if size == PIXEL_4x4 as c_int {
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx: c_int = (*m.offset(0)).mv[0] as c_int + 4 * 2 * 0;
            let mut mvy: c_int = (*m.offset(0)).mv[1] as c_int + 4 * 2 * 0;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 0 + 2 * 0 * 16) as isize),
                16 as intptr_t,
            ...
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix2.offset((2 * 0 + 2 * 0 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            let mut offset: c_int = 0 + (2 >> chroma_v_shift) * 16 * 0;
            let mut chroma_height: c_int = (2 >> chroma_v_shift) * 2;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset(offset as isize),
                &mut *pix2.offset(offset as isize),
                16 as intptr_t,
                &mut *(*p_fref.offset(4))
                    .offset((or + 2 * 0 + (2 >> chroma_v_shift) * 0 * i_stride) as isize),
                i_stride as intptr_t,
                (*m.offset(0)).mv[0] as c_int,
                (2 >> chroma_v_shift) * ((*m.offset(0)).mv[1] as c_int + mvy_offset),
                2,
                chroma_height,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
                (*(*weight.offset(1)).weightfn.offset((2 >> 2) as isize))
                    .expect("non-null function pointer")(
                    &mut *pix1.as_mut_ptr().offset(offset as isize),
                    16 as intptr_t,
                    &mut *pix1.as_mut_ptr().offset(offset as isize),
            ...
            if !(*weight.offset(2)).weightfn.is_null() {
                (*(*weight.offset(2)).weightfn.offset((2 >> 2) as isize))
                    .expect("non-null function pointer")(
                    &mut *pix2.offset(offset as isize),
                    16 as intptr_t,
                    &mut *pix2.offset(offset as isize),
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_0: c_int = (*m.offset(1)).mv[0] as c_int + 4 * 2 * 2;
            let mut mvy_0: c_int = (*m.offset(1)).mv[1] as c_int + 4 * 2 * 0;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 2 + 2 * 0 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix2.offset((2 * 2 + 2 * 0 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            let mut offset_0: c_int = 2 + (2 >> chroma_v_shift) * 16 * 0;
            let mut chroma_height_0: c_int = (2 >> chroma_v_shift) * 2;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset(offset_0 as isize),
                &mut *pix2.offset(offset_0 as isize),
                16 as intptr_t,
                &mut *(*p_fref.offset(4))
                    .offset((or + 2 * 2 + (2 >> chroma_v_shift) * 0 * i_stride) as isize),
                i_stride as intptr_t,
                (*m.offset(1)).mv[0] as c_int,
                (2 >> chroma_v_shift) * ((*m.offset(1)).mv[1] as c_int + mvy_offset),
                2,
                chroma_height_0,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_1: c_int = (*m.offset(2)).mv[0] as c_int + 4 * 2 * 0;
            let mut mvy_1: c_int = (*m.offset(2)).mv[1] as c_int + 4 * 2 * 2;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 0 + 2 * 2 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix2.offset((2 * 0 + 2 * 2 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            let mut offset_1: c_int = 0 + (2 >> chroma_v_shift) * 16 * 2;
            let mut chroma_height_1: c_int = (2 >> chroma_v_shift) * 2;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset(offset_1 as isize),
                &mut *pix2.offset(offset_1 as isize),
                16 as intptr_t,
                &mut *(*p_fref.offset(4))
                    .offset((or + 2 * 0 + (2 >> chroma_v_shift) * 2 * i_stride) as isize),
                i_stride as intptr_t,
                (*m.offset(2)).mv[0] as c_int,
                (2 >> chroma_v_shift) * ((*m.offset(2)).mv[1] as c_int + mvy_offset),
                2,
                chroma_height_1,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_2: c_int = (*m.offset(3)).mv[0] as c_int + 4 * 2 * 2;
            let mut mvy_2: c_int = (*m.offset(3)).mv[1] as c_int + 4 * 2 * 2;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 2 + 2 * 2 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix2.offset((2 * 2 + 2 * 2 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
                    .as_mut_ptr()
                    .offset(i_ref as isize))
            ...
            let mut offset_2: c_int = 2 + (2 >> chroma_v_shift) * 16 * 2;
            let mut chroma_height_2: c_int = (2 >> chroma_v_shift) * 2;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset(offset_2 as isize),
                &mut *pix2.offset(offset_2 as isize),
                16 as intptr_t,
                &mut *(*p_fref.offset(4))
                    .offset((or + 2 * 2 + (2 >> chroma_v_shift) * 2 * i_stride) as isize),
                i_stride as intptr_t,
                (*m.offset(3)).mv[0] as c_int,
                (2 >> chroma_v_shift) * ((*m.offset(3)).mv[1] as c_int + mvy_offset),
                2,
                chroma_height_2,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_3: c_int = (*m_0.offset(0)).mv[0] as c_int + 4 * 2 * 0;
            let mut mvy_3: c_int = (*m_0.offset(0)).mv[1] as c_int + 4 * 2 * 0;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 0 + 2 * 0 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            let mut offset_3: c_int = 0 + (2 >> chroma_v_shift) * 16 * 0;
            let mut chroma_height_3: c_int = (2 >> chroma_v_shift) * 2;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (2 >> chroma_v_shift) * ((*m_0.offset(0)).mv[1] as c_int + mvy_offset),
                4,
                chroma_height_3,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
                (*(*weight.offset(1)).weightfn.offset((4 >> 2) as isize))
                    .expect("non-null function pointer")(
                    &mut *pix1.as_mut_ptr().offset(offset_3 as isize),
                    16 as intptr_t,
                    &mut *pix1.as_mut_ptr().offset(offset_3 as isize),
            ...
            if !(*weight.offset(2)).weightfn.is_null() {
                (*(*weight.offset(2)).weightfn.offset((4 >> 2) as isize))
                    .expect("non-null function pointer")(
                    &mut *pix2.offset(offset_3 as isize),
                    16 as intptr_t,
                    &mut *pix2.offset(offset_3 as isize),
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_4: c_int = (*m_0.offset(1)).mv[0] as c_int + 4 * 2 * 0;
            let mut mvy_4: c_int = (*m_0.offset(1)).mv[1] as c_int + 4 * 2 * 2;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 0 + 2 * 2 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            let mut offset_4: c_int = 0 + (2 >> chroma_v_shift) * 16 * 2;
            let mut chroma_height_4: c_int = (2 >> chroma_v_shift) * 2;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (2 >> chroma_v_shift) * ((*m_0.offset(1)).mv[1] as c_int + mvy_offset),
                4,
                chroma_height_4,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_5: c_int = (*m_1.offset(0)).mv[0] as c_int + 4 * 2 * 0;
            let mut mvy_5: c_int = (*m_1.offset(0)).mv[1] as c_int + 4 * 2 * 0;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 0 + 2 * 0 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            let mut offset_5: c_int = 0 + (2 >> chroma_v_shift) * 16 * 0;
            let mut chroma_height_5: c_int = (2 >> chroma_v_shift) * 4;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (2 >> chroma_v_shift) * ((*m_1.offset(0)).mv[1] as c_int + mvy_offset),
                2,
                chroma_height_5,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
        ...
        if chroma == CHROMA_444 as c_int {
            let mut mvx_6: c_int = (*m_1.offset(1)).mv[0] as c_int + 4 * 2 * 2;
            let mut mvy_6: c_int = (*m_1.offset(1)).mv[1] as c_int + 4 * 2 * 0;
            (*h).mc.mc_luma.expect("non-null function pointer")(
                &mut *pix1.as_mut_ptr().offset((2 * 2 + 2 * 0 * 16) as isize),
                16 as intptr_t,
                &mut *(*(*(*h).mb.pic.p_fref.as_mut_ptr().offset(0))
            ...
            let mut offset_6: c_int = 2 + (2 >> chroma_v_shift) * 16 * 0;
            let mut chroma_height_6: c_int = (2 >> chroma_v_shift) * 4;
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (2 >> chroma_v_shift) * ((*m_1.offset(1)).mv[1] as c_int + mvy_offset),
                2,
                chroma_height_6,
            );
            if !(*weight.offset(1)).weightfn.is_null() {
    ...
    let mut oe: c_int =
        (8 >> chroma_h_shift) * (i8x8 & 1) + (4 >> chroma_v_shift) * (i8x8 & 2) * FENC_STRIDE;
...
unsafe extern "C" fn mb_analyse_inter_p4x4(
    ...
    while i4x4 < 4 {
        ...
        (*m).i_stride[1] = (*h).mb.pic.i_stride[1];
        (*m).i_stride[2] = (*h).mb.pic.i_stride[2];
        (*m).p_fenc[0] = &mut *(*p_fenc.offset(0)).offset((4 * x4 + 4 * y4 * FENC_STRIDE) as isize)
            as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            (*m).p_fenc[1] = &mut *(*p_fenc.offset(1)).offset(
                ((4 * x4 >> (*h).mb.chroma_h_shift)
                    + (4 * y4 >> (*h).mb.chroma_v_shift) * FENC_STRIDE) as isize,
            ) as *mut pixel;
            (*m).p_fenc[2] = &mut *(*p_fenc.offset(2)).offset(
                ((4 * x4 >> (*h).mb.chroma_h_shift)
                    + (4 * y4 >> (*h).mb.chroma_v_shift) * FENC_STRIDE) as isize,
        ...
        (*m).p_fref[0] = &mut *(*p_fref.offset(0))
            .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(0)) as isize)
            as *mut pixel;
        (*m).p_fref_w = (*m).p_fref[0];
        if (*h).param.analyse.i_subpel_refine != 0 {
            (*m).p_fref[1] = &mut *(*p_fref.offset(1))
                .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            (*m).p_fref[2] = &mut *(*p_fref.offset(2))
                .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            (*m).p_fref[3] = &mut *(*p_fref.offset(3))
                .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
        }
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
            (*m).p_fref[4] = &mut *(*p_fref.offset(4))
                .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(1)) as isize)
                as *mut pixel;
            (*m).p_fref[8] = &mut *(*p_fref.offset(8))
                .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(2)) as isize)
                as *mut pixel;
            if (*h).param.analyse.i_subpel_refine != 0 {
                (*m).p_fref[5] = &mut *(*p_fref.offset(5))
                    .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(1)) as isize)
                    as *mut pixel;
                (*m).p_fref[6] = &mut *(*p_fref.offset(6))
                    .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(1)) as isize)
                    as *mut pixel;
                (*m).p_fref[7] = &mut *(*p_fref.offset(7))
                    .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(1)) as isize)
                    as *mut pixel;
                (*m).p_fref[9] = &mut *(*p_fref.offset(9 as isize))
                    .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(2)) as isize)
                    as *mut pixel;
                (*m).p_fref[10] = &mut *(*p_fref.offset(10 as isize))
                    .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(2)) as isize)
                    as *mut pixel;
                (*m).p_fref[11] = &mut *(*p_fref.offset(11 as isize))
                    .offset((4 * x4 + 4 * y4 * *(*m).i_stride.as_mut_ptr().offset(2)) as isize)
                    as *mut pixel;
            }
        } else if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            (*m).p_fref[4] = &mut *(*p_fref.offset(4)).offset(
                (4 * x4
                    + (4 * y4 >> (*h).mb.chroma_v_shift) * *(*m).i_stride.as_mut_ptr().offset(1))
                    as isize,
            ) as *mut pixel;
        }
        if (*h).param.analyse.me_method.exhaustive_search() {
        ...
        (*m).p_fref_w = &mut *(*(*h).mb.pic.p_fref_w.as_mut_ptr().offset(i_ref as isize))
            ...
            as *mut pixel;
        (*m).weight = (*(*h).sh.weight.as_mut_ptr().offset(i_ref as isize)).as_mut_ptr();
        x264_10_mb_predict_mv(h, 0, idx, 1, (*m).mvp.as_mut_ptr());
        x264_10_me_search_ref(
            h,
            m,
...
unsafe extern "C" fn mb_analyse_inter_p8x4(
    ...
    let i_ref: c_int = (*a).l0.me8x8[i8x8 as usize].i_ref;
    (*h).mb.i_partition = D_8x8 as c_int;
    let mut i8x4: c_int = 0;
    while i8x4 < 2 {
        let idx: c_int = 4 * i8x8 + 2 * i8x4;
        let x4: c_int = block_idx_x[idx as usize] as c_int;
        let y4: c_int = block_idx_y[idx as usize] as c_int;
        let i_mvc: c_int = (i8x4 == 0) as c_int;
        ...
        (*m).p_fref_w = &mut *(*(*h).mb.pic.p_fref_w.as_mut_ptr().offset(i_ref as isize))
            ...
            as *mut pixel;
        (*m).weight = (*(*h).sh.weight.as_mut_ptr().offset(i_ref as isize)).as_mut_ptr();
        x264_10_mb_predict_mv(h, 0, idx, 2, (*m).mvp.as_mut_ptr());
        x264_10_me_search_ref(
            h,
            m,
...
unsafe extern "C" fn mb_analyse_inter_p4x8(
    ...
    let i_ref: c_int = (*a).l0.me8x8[i8x8 as usize].i_ref;
    (*h).mb.i_partition = D_8x8 as c_int;
    let mut i4x8: c_int = 0;
    while i4x8 < 2 {
        let idx: c_int = 4 * i8x8 + i4x8;
        let x4: c_int = block_idx_x[idx as usize] as c_int;
        let y4: c_int = block_idx_y[idx as usize] as c_int;
        let i_mvc: c_int = (i4x8 == 0) as c_int;
    ...
    if i_pixel == PIXEL_16x16 as c_int {
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
            ...
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l0.bi16x16.i_stride[1] as intptr_t,
                (*a).l0.bi16x16.mv[0] as c_int,
                2 * ((*a).l0.bi16x16.mv[1] as c_int + l0_mvy_offset) >> v_shift,
                16 >> 1,
                16 >> v_shift,
            );
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l1.bi16x16.i_stride[1] as intptr_t,
                (*a).l1.bi16x16.mv[0] as c_int,
                2 * ((*a).l1.bi16x16.mv[1] as c_int + l1_mvy_offset) >> v_shift,
                16 >> 1,
                16 >> v_shift,
            );
        }
        ...
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
            ...
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l0.me16x8[idx as usize].i_stride[1] as intptr_t,
                (*a).l0.me16x8[idx as usize].mv[0] as c_int,
                2 * ((*a).l0.me16x8[idx as usize].mv[1] as c_int + l0_mvy_offset_0) >> v_shift_0,
                16 >> 1,
                8 >> v_shift_0,
            );
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l1.me16x8[idx as usize].i_stride[1] as intptr_t,
                (*a).l1.me16x8[idx as usize].mv[0] as c_int,
                2 * ((*a).l1.me16x8[idx as usize].mv[1] as c_int + l1_mvy_offset_0) >> v_shift_0,
                16 >> 1,
                8 >> v_shift_0,
            );
        }
        ...
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
            ...
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l0.me8x16[idx as usize].i_stride[1] as intptr_t,
                (*a).l0.me8x16[idx as usize].mv[0] as c_int,
                2 * ((*a).l0.me8x16[idx as usize].mv[1] as c_int + l0_mvy_offset_1) >> v_shift_1,
                8 >> 1,
                16 >> v_shift_1,
            );
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l1.me8x16[idx as usize].i_stride[1] as intptr_t,
                (*a).l1.me8x16[idx as usize].mv[0] as c_int,
                2 * ((*a).l1.me8x16[idx as usize].mv[1] as c_int + l1_mvy_offset_1) >> v_shift_1,
                8 >> 1,
                16 >> v_shift_1,
            );
        }
        ...
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
            ...
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l0.me8x8[idx as usize].i_stride[1] as intptr_t,
                (*a).l0.me8x8[idx as usize].mv[0] as c_int,
                2 * ((*a).l0.me8x8[idx as usize].mv[1] as c_int + l0_mvy_offset_2) >> v_shift_2,
                8 >> 1,
                8 >> v_shift_2,
            );
            (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                (*a).l1.me8x8[idx as usize].i_stride[1] as intptr_t,
                (*a).l1.me8x8[idx as usize].mv[0] as c_int,
                2 * ((*a).l1.me8x8[idx as usize].mv[1] as c_int + l1_mvy_offset_2) >> v_shift_2,
                8 >> 1,
                8 >> v_shift_2,
            );
        }
...
unsafe extern "C" fn mb_analyse_inter_b16x16(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    let mut p_halfpel_thresh: [*mut c_int; 2] = [
        ...
        if (*a).b_early_terminate != 0 && (*h).mb.pic.i_fref[1] > 1 {
    ...
    while l >= 0 {
        ...
        i_ref = if list1_skipped != 0 && l == 1 { 1 } else { 0 };
        while i_ref < (*h).mb.pic.i_fref[l as usize] {
            if try_skip != 0 && l == 1 && i_ref > 0 {
                list1_skipped = 1;
                break;
            } else {
                ...
                if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
                    ...
                    .offset(4))
                    .offset(
                        (0 + (0 >> (*h).mb.chroma_v_shift) * *m.i_stride.as_mut_ptr().offset(1))
                            as isize,
                    ) as *mut pixel;
                }
                if (*h).param.analyse.me_method.exhaustive_search() {
        ...
        if list1_skipped != 0 && l == 1 && i_ref == (*h).mb.pic.i_fref[1] {
            break;
        }
        if list1_skipped != 0 && l == 0 {
            l = 1;
        } else {
            l -= 1;
        }
    ...
    if (*h).mb.b_chroma_me != 0 {
        (*a).i_cost16x16bi += analyse_bi_chroma(h, a, 0, PIXEL_16x16 as c_int);
    }
        ...
        != 0
    {
        ...
        if (*h).mb.b_chroma_me != 0 && cost00 < (*a).i_cost16x16bi {
            ...
            if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc == CHROMA_444 as c_int {
                ...
                let mut v_shift: c_int = (*h).mb.chroma_v_shift;
                if v_shift & (*h).mb.interlaced as i32 & (*a).l0.bi16x16.i_ref != 0 {
                    let mut l0_mvy_offset: c_int = ((*h).mb.i_mb_y & 1) * 4 - 2;
                    (*h).mc.mc_chroma.expect("non-null function pointer")(
                ...
                if v_shift & (*h).mb.interlaced as i32 & (*a).l1.bi16x16.i_ref != 0 {
                    let mut l1_mvy_offset: c_int = ((*h).mb.i_mb_y & 1) * 4 - 2;
                    (*h).mc.mc_chroma.expect("non-null function pointer")(
...
unsafe extern "C" fn mb_load_mv_direct8x8(mut h: *mut x264_t, mut idx: c_int) {
    let mut x: c_int = 2 * (idx & 1);
    let mut y: c_int = idx & 2;
    x264_macroblock_cache_ref(h, x, y, 2, 2, 0, (*h).mb.cache.direct_ref[0][idx as usize]);
    x264_macroblock_cache_ref(h, x, y, 2, 2, 1, (*h).mb.cache.direct_ref[1][idx as usize]);
    x264_macroblock_cache_mv(
        h,
        x,
        y,
...
unsafe extern "C" fn mb_cache_mv_b8x8(
    ...
    if (*h).mb.i_sub_partition[i as usize] as c_int == D_DIRECT_8x8 as c_int {
        ...
        if b_mvd != 0 {
            x264_macroblock_cache_mvd(h, x, y, 2, 2, 0, 0 as uint16_t);
            x264_macroblock_cache_mvd(h, x, y, 2, 2, 1, 0 as uint16_t);
            x264_macroblock_cache_skip(h, x, y, 2, 2, 1);
        }
    } else {
        if x264_mb_partition_listX_table[0][(*h).mb.i_sub_partition[i as usize] as usize] != 0 {
            x264_macroblock_cache_ref(h, x, y, 2, 2, 0, (*a).l0.me8x8[i as usize].i_ref as int8_t);
            x264_macroblock_cache_mv(
                h,
                x,
                y,
            ...
            x264_macroblock_cache_ref(h, x, y, 2, 2, 0, -1 as int8_t);
            x264_macroblock_cache_mv(h, x, y, 2, 2, 0, 0 as uint32_t);
            if b_mvd != 0 {
                x264_macroblock_cache_mvd(h, x, y, 2, 2, 0, 0 as uint16_t);
            }
        }
        if x264_mb_partition_listX_table[1][(*h).mb.i_sub_partition[i as usize] as usize] != 0 {
            x264_macroblock_cache_ref(h, x, y, 2, 2, 1, (*a).l1.me8x8[i as usize].i_ref as int8_t);
            x264_macroblock_cache_mv(
                h,
                x,
                y,
            ...
            x264_macroblock_cache_ref(h, x, y, 2, 2, 1, -1 as int8_t);
            x264_macroblock_cache_mv(h, x, y, 2, 2, 1, 0 as uint32_t);
            if b_mvd != 0 {
                x264_macroblock_cache_mvd(h, x, y, 2, 2, 1, 0 as uint16_t);
            }
        }
...
unsafe extern "C" fn mb_cache_mv_b16x8(
    ...
    if x264_mb_partition_listX_table[0][(*a).i_mb_partition16x8[i as usize] as usize] != 0 {
        ...
        x264_macroblock_cache_ref(h, 0, 2 * i, 4, 2, 0, -1 as int8_t);
        x264_macroblock_cache_mv(h, 0, 2 * i, 4, 2, 0, 0 as uint32_t);
        if b_mvd != 0 {
            x264_macroblock_cache_mvd(h, 0, 2 * i, 4, 2, 0, 0 as uint16_t);
        }
    }
    if x264_mb_partition_listX_table[1][(*a).i_mb_partition16x8[i as usize] as usize] != 0 {
        ...
        x264_macroblock_cache_ref(h, 0, 2 * i, 4, 2, 1, -1 as int8_t);
        x264_macroblock_cache_mv(h, 0, 2 * i, 4, 2, 1, 0 as uint32_t);
        if b_mvd != 0 {
            x264_macroblock_cache_mvd(h, 0, 2 * i, 4, 2, 1, 0 as uint16_t);
        }
    };
}
#[inline]
...
unsafe extern "C" fn mb_cache_mv_b8x16(
    ...
    if x264_mb_partition_listX_table[0][(*a).i_mb_partition8x16[i as usize] as usize] != 0 {
        ...
        x264_macroblock_cache_ref(h, 2 * i, 0, 2, 4, 0, -1 as int8_t);
        x264_macroblock_cache_mv(h, 2 * i, 0, 2, 4, 0, 0 as uint32_t);
        if b_mvd != 0 {
            x264_macroblock_cache_mvd(h, 2 * i, 0, 2, 4, 0, 0 as uint16_t);
        }
    }
    if x264_mb_partition_listX_table[1][(*a).i_mb_partition8x16[i as usize] as usize] != 0 {
        ...
        x264_macroblock_cache_ref(h, 2 * i, 0, 2, 4, 1, -1 as int8_t);
        x264_macroblock_cache_mv(h, 2 * i, 0, 2, 4, 1, 0 as uint32_t);
        if b_mvd != 0 {
            x264_macroblock_cache_mvd(h, 2 * i, 0, 2, 4, 1, 0 as uint16_t);
        }
    };
}
...
unsafe extern "C" fn mb_analyse_inter_b8x8_mixed_ref(
    ...
    let mut i_maxref: [c_int; 2] = [(*h).mb.pic.i_fref[0] - 1, (*h).mb.pic.i_fref[1] - 1];
    let mut l: c_int = 0;
    while l < 2 {
        let mut lX: *mut x264_mb_analysis_list_t = if l != 0 { &mut (*a).l1 } else { &mut (*a).l0 };
        if i_maxref[l as usize] > 0
            && (*lX).me16x16.i_ref == 0
            && (*h).mb.i_mb_type_top > 0
            && (*h).mb.i_mb_type_left[0] > 0
        {
            i_maxref[l as usize] = 0;
            let mut ref_0: c_int =
                (*h).mb.cache.ref_0[l as usize][(X264_SCAN8_0 + -(8) - 1) as usize] as c_int;
            if ref_0 > i_maxref[l as usize] {
                i_maxref[l as usize] = ref_0;
            }
            let mut ref_1: c_int =
                (*h).mb.cache.ref_0[l as usize][(X264_SCAN8_0 + -(8) + 0) as usize] as c_int;
            if ref_1 > i_maxref[l as usize] {
                i_maxref[l as usize] = ref_1;
            }
            let mut ref_2: c_int =
                (*h).mb.cache.ref_0[l as usize][(X264_SCAN8_0 + -(8) + 2) as usize] as c_int;
            if ref_2 > i_maxref[l as usize] {
                i_maxref[l as usize] = ref_2;
            }
            let mut ref_3: c_int =
                (*h).mb.cache.ref_0[l as usize][(X264_SCAN8_0 + -(8) + 4) as usize] as c_int;
            if ref_3 > i_maxref[l as usize] {
                i_maxref[l as usize] = ref_3;
            }
            let mut ref_4: c_int =
                (*h).mb.cache.ref_0[l as usize][(X264_SCAN8_0 + 0 - 1) as usize] as c_int;
            if ref_4 > i_maxref[l as usize] {
                i_maxref[l as usize] = ref_4;
            }
            let mut ref_5: c_int =
                (*h).mb.cache.ref_0[l as usize][(X264_SCAN8_0 + 2 * 8 - 1) as usize] as c_int;
    ...
    while i < 4 {
        ...
        let mut stride: [intptr_t; 2] = [8 as intptr_t, 8 as intptr_t];
        ...
        m.i_stride[2] = (*h).mb.pic.i_stride[2];
        m.p_fenc[0] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(0))
            .offset((8 * x8 + 8 * y8 * FENC_STRIDE) as isize) as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            m.p_fenc[1] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(1)).offset(
                ((8 * x8 >> (*h).mb.chroma_h_shift)
                    + (8 * y8 >> (*h).mb.chroma_v_shift) * FENC_STRIDE) as isize,
        ...
        while l_0 < 2 {
            ...
            while i_ref <= i_maxref[l_0 as usize] {
                ...
                m.weight = x264_zero.as_mut_ptr() as *const x264_weight_t;
                m.i_ref = i_ref;
                x264_macroblock_cache_ref(h, x8 * 2, y8 * 2, 2, 2, l_0, i_ref as int8_t);
                x264_10_mb_predict_mv(h, l_0, 4 * i, 2, m.mvp.as_mut_ptr());
                x264_10_me_search_ref(
                    h,
                    &mut m,
...
unsafe extern "C" fn mb_analyse_inter_b8x8(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    while i < 4 {
        ...
        while l < 2 {
            ...
            (*m).i_stride[2] = (*h).mb.pic.i_stride[2];
            (*m).p_fenc[0] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(0))
                .offset((8 * x8 + 8 * y8 * FENC_STRIDE) as isize)
                as *mut pixel;
            if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
                (*m).p_fenc[1] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(1)).offset(
                    ((8 * x8 >> (*h).mb.chroma_h_shift)
                        + (8 * y8 >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
                        as isize,
                ) as *mut pixel;
                (*m).p_fenc[2] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(2)).offset(
                    ((8 * x8 >> (*h).mb.chroma_h_shift)
            ...
            (*m).p_fref[0] = &mut *(*(*p_fref.as_mut_ptr().offset(l as isize)).offset(0))
                .offset((8 * x8 + 8 * y8 * *(*m).i_stride.as_mut_ptr().offset(0)) as isize)
                as *mut pixel;
            (*m).p_fref_w = (*m).p_fref[0];
            if (*h).param.analyse.i_subpel_refine != 0 {
            ...
            (*m).weight = x264_zero.as_mut_ptr() as *const x264_weight_t;
            (*m).i_ref = (*lX).me16x16.i_ref;
            x264_macroblock_cache_ref(h, x8 * 2, y8 * 2, 2, 2, l, (*lX).me16x16.i_ref as int8_t);
            x264_10_mb_predict_mv(h, l, 4 * i, 2, (*m).mvp.as_mut_ptr());
            x264_10_me_search_ref(h, m, &mut (*lX).me16x16.mv, 1, 0 as *mut c_int);
...
unsafe extern "C" fn mb_analyse_inter_b16x8(
    ...
    while i < 2 {
        let mut i_part_cost: c_int = 0;
        let mut i_part_cost_bi: c_int = 0;
        let mut stride: [intptr_t; 2] = [16 as intptr_t, 16 as intptr_t];
        let mut src: [*mut pixel; 2] = [0 as *mut pixel; 2];
        let mut m: x264_me_t = x264_me_t {
            i_pixel: 0,
            p_cost_mv: 0 as *mut uint16_t,
        ...
        m.i_stride[2] = (*h).mb.pic.i_stride[2];
        m.p_fenc[0] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(0))
            .offset((0 + 8 * i * FENC_STRIDE) as isize) as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            m.p_fenc[1] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(1)).offset(
                ((0 >> (*h).mb.chroma_h_shift) + (8 * i >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
                    as isize,
        ...
        while l < 2 {
            ...
            let mut ref8: [c_int; 2] = [
                (*lX).me8x8[(2 * i) as usize].i_ref,
                (*lX).me8x8[(2 * i + 1) as usize].i_ref,
            ];
            let mut i_ref8s: c_int = if ref8[0] == ref8[1] { 1 } else { 2 };
            ...
            while j < i_ref8s {
                ...
                (*((*mvc.as_mut_ptr().offset(2)).as_mut_ptr() as *mut x264_union32_t)).i =
                    ...
                    .as_mut_ptr() as *mut x264_union32_t))
                        .i;
                x264_macroblock_cache_ref(h, 0, 2 * i, 4, 2, l, i_ref as int8_t);
                x264_10_mb_predict_mv(h, l, 8 * i, 4, m.mvp.as_mut_ptr());
                x264_10_me_search_ref(
                    h,
                    &mut m,
                    mvc.as_mut_ptr() as *mut [int16_t; 2],
        ...
        if (*a).b_early_terminate != 0
            && (i == 0
                && i_part_cost + (*a).i_cost_est16x8[1]
                    > i_best_satd
                        * (16 + (((*a).i_mbrd != 0) as c_int + ((*h).mb.i_psy_rd != 0) as c_int))
                        / 16)
        {
...
unsafe extern "C" fn mb_analyse_inter_b8x16(
    ...
    while i < 2 {
        ...
        m.i_stride[2] = (*h).mb.pic.i_stride[2];
        m.p_fenc[0] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(0))
            .offset((8 * i + 0 * FENC_STRIDE) as isize) as *mut pixel;
        if (*(*h).sps.as_mut_ptr()).i_chroma_format_idc != 0 {
            m.p_fenc[1] = &mut *(*(*h).mb.pic.p_fenc.as_mut_ptr().offset(1)).offset(
                ((8 * i >> (*h).mb.chroma_h_shift) + (0 >> (*h).mb.chroma_v_shift) * FENC_STRIDE)
                    as isize,
        ...
        while l < 2 {
            ...
            while j < i_ref8s {
                ...
                (*((*mvc.as_mut_ptr().offset(2)).as_mut_ptr() as *mut x264_union32_t)).i =
                    ...
                    .as_mut_ptr() as *mut x264_union32_t))
                        .i;
                x264_macroblock_cache_ref(h, 2 * i, 0, 2, 4, l, i_ref as int8_t);
                x264_10_mb_predict_mv(h, l, 4 * i, 2, m.mvp.as_mut_ptr());
                x264_10_me_search_ref(
                    h,
                    &mut m,
...
unsafe extern "C" fn mb_analyse_p_rd(
    ...
    if (*a).l0.i_rd16x16 == COST_MAX
        && ((*a).b_early_terminate == 0 || (*a).l0.me16x16.cost <= i_satd * 3 / 2)
    {
        (*h).mb.i_partition = D_16x16 as c_int;
        analyse_update_cache(h, a);
        (*a).l0.i_rd16x16 = rd_cost_mb(h, (*a).i_lambda2);
    ...
    if (*a).l0.i_cost8x8 < thresh {
        ...
        if (*h).param.analyse.inter & X264_ANALYSE_PSUB8x8 != 0 {
            x264_macroblock_cache_ref(h, 0, 0, 2, 2, 0, (*a).l0.me8x8[0].i_ref as int8_t);
            x264_macroblock_cache_ref(h, 2, 0, 2, 2, 0, (*a).l0.me8x8[1].i_ref as int8_t);
            x264_macroblock_cache_ref(h, 0, 2, 2, 2, 0, (*a).l0.me8x8[2].i_ref as int8_t);
            x264_macroblock_cache_ref(h, 2, 2, 2, 2, 0, (*a).l0.me8x8[3].i_ref as int8_t);
            let mut i: c_int = 0;
            while i < 4 {
                ...
                while subtype <= D_L0_8x8 as c_int {
                    ...
                    if !(costs[subtype as usize] > sub8x8_thresh) {
                        ...
                        if !(subtype == btype) {
                            cost =
                                x264_10_rd_cost_part(h, (*a).i_lambda2, i << 2, PIXEL_8x8 as c_int);
                            if cost < bcost {
                                bcost = cost;
                                btype = subtype;
                            }
...
unsafe extern "C" fn mb_analyse_b_rd(
    ...
    let mut thresh: c_int = if (*a).b_early_terminate != 0 {
        i_satd_inter * (17 + ((*h).mb.i_psy_rd != 0) as c_int) / 16 + 1
    } else {
        COST_MAX
    };
...
unsafe extern "C" fn mb_analyse_transform(mut h: *mut x264_t) {
        ...
        && !(*h).mb.lossless
    {
        ...
        let mut i_cost8: c_int = 0;
        let mut i_cost4: c_int = 0;
        if (*h).pixf.sa8d_satd[PIXEL_16x16 as c_int as usize].is_some() {
            ...
            i_cost4 = (cost >> 32) as uint32_t as c_int;
        } else {
            let mut p_0: c_int = 0;
            while p_0 < plane_count {
...
unsafe extern "C" fn mb_analyse_qp_rd(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    ...
    while direction >= -1 {
        ...
        if direction == -1 {
            if origcbp == 0 {
                (*h).mb.i_qp = if (*h).mb.i_qp - threshold - 1
                    > (if (*h).param.rc.i_qp_min < 51 + 6 * (10 - 8) {
                        (*h).param.rc.i_qp_min
                    } else {
                        51 + 6 * (10 - 8)
                    }) {
                    (*h).mb.i_qp - threshold - 1
                } else if (*h).param.rc.i_qp_min < 51 + 6 * (10 - 8) {
                    (*h).param.rc.i_qp_min
                } else {
                    51 + 6 * (10 - 8)
                };
                ...
                })
        {
            ...
            if direction == 1 && *(*h).mb.cbp.offset((*h).mb.i_mb_xy as isize) == 0 {
                break;
            }
            (*h).mb.i_qp += direction;
        }
...
pub unsafe extern "C" fn x264_10_macroblock_analyse(mut h: *mut x264_t) {
    ...
    if (*h).param.rc.i_aq_mode != 0 && (*h).param.analyse.i_subpel_refine < 10 {
        (*h).mb.i_qp = if abs((*h).mb.i_qp - (*h).mb.i_last_qp) == 1 {
            (*h).mb.i_last_qp
        } else {
            (*h).mb.i_qp
        };
    ...
    if (*h).sh.i_type == SLICE_TYPE_I as c_int {
        ...
        let mut b_skip: c_int = 0;
        (*h).mc.prefetch_ref.expect("non-null function pointer")(
            (*h).mb.pic.p_fref[0][0][((*h).mb.i_mb_x & 3) as usize],
            (*h).mb.pic.i_stride[0] as intptr_t,
            0,
        );
        ...
        if analysis.b_force_intra != 0 {
            ...
            match current_block {
                ...
                _ => {
                    let mut skip_invalid: c_int = ((*h).i_thread_frames > 1
                        ...
                        && skip_invalid == 0
                    {
                        ...
                        if !(skip_invalid != 0) {
                            ...
                            {
                                b_skip = x264_10_macroblock_probe_skip(h, 0);
                            }
                        }
                    }
                    current_block = 13460095289871124136;
                    ...
                    13460095289871124136 => {
                        ...
                        if b_skip != 0 {
                            ...
                            if analysis.i_mbrd != 0 {
                                ...
                                intra_rd(h, &mut analysis, i_satd_inter * 5 / 4 + 1);
                            }
                                    ...
                                    || i_type == I_PCM as c_int)
                            {
                                ...
                                if analysis.i_mbrd >= 2 && (*h).mb.i_type != I_PCM as c_int {
                                    if (*h).mb.i_type == I_4x4 as c_int
                                        || (*h).mb.i_type == I_8x8 as c_int
                                        || (*h).mb.i_type == I_16x16 as c_int
                                        || (*h).mb.i_type == I_PCM as c_int
        ...
        if (*h).sh.i_type == SLICE_TYPE_B as c_int {
            ...
            let mut b_skip_0: c_int = 0;
            if analysis.i_mbrd != 0 {
                mb_init_fenc_cache(h, (analysis.i_mbrd >= 2) as c_int);
            }
            (*h).mb.i_type = B_SKIP as c_int;
            if (*h).mb.b_direct_auto_write != 0 {
                let mut i_2: c_int = 0;
                while i_2 < 2 {
                    let mut b_changed: c_int = 1;
                    ...
                    if analysis.b_direct_available != 0 {
                        if b_changed != 0 {
                            x264_10_mb_mc(h);
                            b_skip_0 = x264_10_macroblock_probe_skip(h, 1);
                        }
            ...
            if analysis.b_direct_available != 0 {
                    ...
                    && (*h).mb.i_mb_y * 16 >= (*h).param.height as i32
                {
                    ...
                    (*h).mb.b_skip_mc =
                        (i_bskip_cost <= 6 * analysis.i_lambda2 + 128 >> 8) as c_int;
                    b_skip_0 = (*h).mb.b_skip_mc;
                } else if (*h).mb.b_direct_auto_write == 0 {
                    analysis.b_try_skip = x264_10_macroblock_probe_skip(h, 1);
                    if (*h).param.analyse.i_subpel_refine < 3 {
            ...
            if b_skip_0 == 0 {
                ...
                if analysis.i_mbrd != 0
                    && analysis.b_early_terminate != 0
                    && analysis.i_cost16x16direct <= i_cost * 33 / 32
                {
                ...
                if flags_0 & X264_ANALYSE_BSUB16x16 != 0 {
                    ...
                    let mut i_cost_est16x8bi_total: c_int = 0;
                    let mut i_cost_est8x16bi_total: c_int = 0;
                    ...
                    while i_7 < 2 {
                        ...
                        i_l0_satd = analysis.i_satd8x8[0][(i_7 * 2) as usize]
                            + analysis.i_satd8x8[0][(i_7 * 2 + 1) as usize];
                        i_l1_satd = analysis.i_satd8x8[1][(i_7 * 2) as usize]
                            + analysis.i_satd8x8[1][(i_7 * 2 + 1) as usize];
                        i_bi_satd = analysis.i_satd8x8[2][(i_7 * 2) as usize]
                            + analysis.i_satd8x8[2][(i_7 * 2 + 1) as usize];
                        avg_l0_mv_ref_cost = analysis.l0.me8x8[(i_7 * 2) as usize].cost_mv
                            + analysis.l0.me8x8[(i_7 * 2) as usize].i_ref_cost
                            + analysis.l0.me8x8[(i_7 * 2 + 1) as usize].cost_mv
                            + analysis.l0.me8x8[(i_7 * 2 + 1) as usize].i_ref_cost
                            + 1
                            >> 1;
                        avg_l1_mv_ref_cost = analysis.l1.me8x8[(i_7 * 2) as usize].cost_mv
                            + analysis.l1.me8x8[(i_7 * 2) as usize].i_ref_cost
                            + analysis.l1.me8x8[(i_7 * 2 + 1) as usize].cost_mv
                            + analysis.l1.me8x8[(i_7 * 2 + 1) as usize].i_ref_cost
                            + 1
                            >> 1;
                ...
                if analysis.i_mbrd != 0 {
                    ...
                    intra_rd(h, &mut analysis, i_satd_inter_0 * 17 / 16 + 1);
                }
                if analysis.i_satd_i16x16 < i_cost {
                    i_cost = analysis.i_satd_i16x16;
                    ...
                    && i_type_0 < B_SKIP as c_int
                {
                    ...
                    if i_partition_0 == D_16x16 as c_int {
                        ...
                        let mut i_11: c_int = 0;
                        while i_11 < 2 {
                            (*h).mb.i_sub_partition[(i_11 * 2 + 1) as usize] =
                                analysis.i_mb_partition16x8[i_11 as usize] as uint8_t;
                            (*h).mb.i_sub_partition[(i_11 * 2) as usize] =
                                (*h).mb.i_sub_partition[(i_11 * 2 + 1) as usize];
                        ...
                        let mut i_12: c_int = 0;
                        while i_12 < 2 {
                            (*h).mb.i_sub_partition[(i_12 + 2) as usize] =
    ...
    if analysis.i_mbrd >= 2 {
        static mut check_mv_lists: [uint8_t; 19] =
            [0, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 2, 0, 0, 0, 0, 0, 0];
        let mut list: c_int = check_mv_lists[(*h).mb.i_type as usize] as c_int - 1;
        if list >= 0
            && (*h).mb.i_partition != D_16x16 as c_int
            && (*(&mut *(*(*h).mb.cache.mv.as_mut_ptr().offset(list as isize))
                ...
                == (*(&mut *(*(*h).mb.cache.mv.as_mut_ptr().offset(list as isize))
                    .as_mut_ptr()
                    .offset(*x264_scan8.as_ptr().offset(12 as isize) as isize)
                    as *mut [int16_t; 2] as *mut x264_union32_t))
                    .i
    ...
    if (*h).mb.b_trellis == 1 || (*h).mb.b_noise_reduction != 0 {
        (*h).mb.i_skip_intra = 0;
    }
}
...
unsafe extern "C" fn analyse_update_cache(mut h: *mut x264_t, mut a: *mut x264_mb_analysis_t) {
    match (*h).mb.i_type {
        ...
        4 => match (*h).mb.i_partition {
            ...
            14 => {
                x264_macroblock_cache_ref(h, 0, 0, 4, 2, 0, (*a).l0.me16x8[0].i_ref as int8_t);
                x264_macroblock_cache_ref(h, 0, 2, 4, 2, 0, (*a).l0.me16x8[1].i_ref as int8_t);
                x264_macroblock_cache_mv(
                    h,
                    0,
                    0,
            ...
            15 => {
                x264_macroblock_cache_ref(h, 0, 0, 2, 4, 0, (*a).l0.me8x16[0].i_ref as int8_t);
                x264_macroblock_cache_ref(h, 2, 0, 2, 4, 0, (*a).l0.me8x16[1].i_ref as int8_t);
                x264_macroblock_cache_mv(
                    h,
                    0,
                    0,
        ...
        6 => {
            (*h).mb.i_partition = D_16x16 as c_int;
            x264_macroblock_cache_ref(h, 0, 0, 4, 4, 0, 0 as int8_t);
            x264_macroblock_cache_mv(
                h,
                0,
                0,
        ...
        _ => match (*h).mb.i_partition {
            16 => match (*h).mb.i_type {
                8 => {
                    ...
                    x264_macroblock_cache_ref(h, 0, 0, 4, 4, 1, -1 as int8_t);
                    x264_macroblock_cache_mv(h, 0, 0, 4, 4, 1, 0 as uint32_t);
                    x264_macroblock_cache_mvd(h, 0, 0, 4, 4, 1, 0 as uint16_t);
                }
                12 => {
                    x264_macroblock_cache_ref(h, 0, 0, 4, 4, 0, -1 as int8_t);
                    x264_macroblock_cache_mv(h, 0, 0, 4, 4, 0, 0 as uint32_t);
                    x264_macroblock_cache_mvd(h, 0, 0, 4, 4, 0, 0 as uint16_t);
                    x264_macroblock_cache_ref(h, 0, 0, 4, 4, 1, (*a).l1.me16x16.i_ref as int8_t);
                    x264_macroblock_cache_mv(
                        h,
                        0,
                        0,
                ...
                16 => {
                    x264_macroblock_cache_ref(h, 0, 0, 4, 4, 0, (*a).l0.bi16x16.i_ref as int8_t);
                    x264_macroblock_cache_mv(
                        h,
                        0,
                        0,
                    ...
                    x264_macroblock_cache_ref(h, 0, 0, 4, 4, 1, (*a).l1.bi16x16.i_ref as int8_t);
                    x264_macroblock_cache_mv(
                        h,
                        0,
                        0,
