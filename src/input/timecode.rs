...
unsafe extern "C" fn sigexp10(mut value: c_double, mut exponent: *mut c_double) -> c_double {
    *exponent = pow(10 as c_double, floor(log10(value)));
    return value / *exponent;
}
#[c2rust::src_loc = "51:9"]
const DOUBLE_EPSILON: c_double = 5e-6f64;
#[c2rust::src_loc = "52:9"]
const MKV_TIMEBASE_DEN: c_int = 1000000000;
#[c2rust::src_loc = "54:1"]
    ...
    if tcfv == 1 {
        ...
        .is_null()
        {
                ...
                || buff[0] as c_int == '\r' as i32
            {
                ...
                if start > end || start <= prev_start || end <= prev_end || seq_fps <= 0 as c_double
                {
                    x264_cli_log(
                        b"timecode\0" as *const u8 as *const c_char,
                        X264_LOG_ERROR,
        ...
        if (*h).auto_timebase_den != 0 || (*h).auto_timebase_num != 0 {
            fpss = malloc(((seq_num + 1) as size_t).wrapping_mul(size_of::<c_double>() as size_t))
                as *mut c_double;
            if fpss.is_null() {
        ...
        match current_block {
            ...
            _ => {
                ...
                if assume_fps < 0 as c_double {
                    current_block = 15792084957793291482;
                } else {
                    *timecodes.offset(0) = 0 as c_double;
                    seq_num = 0;
                    num = seq_num;
                    loop {
                        ...
                        while num < start && num < timecodes_num - 1 {
                            *timecodes.offset((num + 1) as isize) =
                                *timecodes.offset(num as isize) + 1 as c_double / assume_fps;
                            num += 1;
                        }
                        if !(num < timecodes_num - 1) {
                            continue;
                        ...
                        if seq_fps < 0 as c_double {
                            current_block = 15792084957793291482;
                            break;
                        }
                        num = start;
                        while num <= end && num < timecodes_num - 1 {
                            *timecodes.offset((num + 1) as isize) =
                                *timecodes.offset(num as isize) + 1 as c_double / seq_fps;
                            num += 1;
                        }
                    }
                    match current_block {
                        ...
                        _ => {
                                        ...
                                        .is_null()
                                    {
                                        ...
                                        seq_fps = MKV_TIMEBASE_DEN as c_double
                                            / (round(MKV_TIMEBASE_DEN as c_double / seq_fps_sig)
                                                / exponent);
                                        while num < start && num < timecodes_num - 1 {
                                            *timecodes.offset((num + 1) as isize) = *timecodes
                                                .offset(num as isize)
                                                + 1 as c_double / assume_fps;
                                            num += 1;
                                        }
                                        num = start;
                                        while num <= end && num < timecodes_num - 1 {
                            ...
                            match current_block {
                                ...
                                _ => {
                                    ...
                                    (*h).last_timecode =
                                        *timecodes.offset((timecodes_num - 1) as isize);
            ...
            *timecodes.offset(0) *= 1e-3f64;
                ...
                .is_null()
            {
                ...
                *timecodes.offset(num as isize) *= 1e-3f64;
                if ret != 1
                    || *timecodes.offset(num as isize) <= *timecodes.offset((num - 1) as isize)
                {
                    x264_cli_log(
                        b"timecode\0" as *const u8 as *const c_char,
                        X264_LOG_ERROR,
        ...
        if timecodes_num == 1 {
            ...
            fpss = malloc(
                ((timecodes_num - 1) as size_t).wrapping_mul(size_of::<c_double>() as size_t),
            ) as *mut c_double;
            if fpss.is_null() {
                current_block = 15792084957793291482;
            } else {
                num = 0;
                while num < timecodes_num - 1 {
                    *fpss.offset(num as isize) = 1 as c_double
                        / (*timecodes.offset((num + 1) as isize) - *timecodes.offset(num as isize));
                    if (*h).auto_timebase_den != 0 {
                        let mut i: c_int = 1;
                        let mut fps_num: uint64_t = 0;
                        let mut fps_den: uint64_t = 0;
        ...
        match current_block {
            ...
            _ => {
                if timecodes_num > 1 {
                    (*h).assume_fps = 1 as c_double
                        / (*timecodes.offset((timecodes_num - 1) as isize)
                            - *timecodes.offset((timecodes_num - 2) as isize));
                } else {
                    (*h).assume_fps = (*info).fps_num as c_double / (*info).fps_den as c_double;
                }
                (*h).last_timecode = *timecodes.offset((timecodes_num - 1) as isize);
                current_block = 12129449210080749085;
            }
        }
    }
                ...
                while num < (*h).stored_pts_num {
                    *(*h).pts.offset(num as isize) = (*timecodes.offset(num as isize)
                        * ((*h).timebase_den as c_double / (*h).timebase_num as c_double)
                        + 0.5f64) as int64_t;
                    if num > 0
                        && *(*h).pts.offset(num as isize) <= *(*h).pts.offset((num - 1) as isize)
                    {
                        x264_cli_log(
                            b"timecode\0" as *const u8 as *const c_char,
                            X264_LOG_ERROR,
...
unsafe extern "C" fn open_file(
    ...
    if !(*opt).timebase.is_null() {
        ...
        if ret == 1 {
            (*h).timebase_num = strtoul((*opt).timebase, 0 as *mut *mut c_char, 10) as uint64_t;
    ...
    (*h).auto_timebase_den = (ret < 2) as c_int;
...
unsafe extern "C" fn get_frame_pts(
    ...
    if frame < (*h).stored_pts_num {
        ...
        let mut timecode: c_double = (*h).last_timecode + 1 as c_double / (*h).assume_fps;
        if real_frame != 0 {
